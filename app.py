...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Tab 1 — Student Analysis  : RF + SHAP + KMeans Clustering + AI Report
Tab 2 — Job Market NLP    : TF-IDF + BERT (optional) + AI Report
Each tab is FULLY INDEPENDENT with its own Generate Report button.
"""
...
with st.sidebar:
    ...
    st.markdown("No need to run both.")
...
def metric_card(col, label, value):
    col.markdown(f'<div class="mc"><div class="v">{value}</div><div class="l">{label}</div></div>',
                 unsafe_allow_html=True)
...
with tab1:
    st.markdown('<p class="sec">01 · Student Analysis — RF + SHAP + KMeans + AI Report</p>',
                unsafe_allow_html=True)
    st.markdown('<div class="bb">Upload <b>any</b> grades CSV. Columns auto-detected. '
                'Model trains on 80%, tests on 20%. No data leakage. '
                'Generate Report button at the bottom creates a full AI institutional report.</div>',
                unsafe_allow_html=True)
    ...
    if up:
        ...
        c1, c2 = st.columns(2)
        with c1:
            course_cols = st.multiselect("Course columns (model features):", num_cols, default=auto_c)
        with c2:
            final_col = st.selectbox("Final grade column (target):", num_cols,
        ...
        if st.button("🔍 Run Analysis", key="b1_run"):
            ...
            rf.fit(Xtr, ytr); ypr = rf.predict(Xte)
            ...
            with st.spinner("Running KMeans Clustering..."):
                ...
                for k in ks:
                    ...
                    silhs.append(silhouette_score(Xs, lb) if k > 1 else 0)
            ...
            st.caption(f"Train: {len(Xtr)} (80%) · Test: {len(Xte)} (20%) · "
                       f"5-fold CV on train · class_weight=balanced")
            mcols = st.columns(6)
            ...
            with sh_c:
                ...
                if shap_v is not None:
                    ...
                    ax5.set_title('SHAP — model explanation', color='#94a3b8', fontsize=8)
...
|--------|-------|---------|
| Accuracy  | {acc*100:.1f}% | (TP+TN) / all |
| Precision | {prec*100:.1f}% | TP / (TP+FP) |
| Recall    | {rec*100:.1f}% | TP / (TP+FN) |
| F1        | {f1*100:.1f}% | 2·P·R/(P+R) |
| CV Acc.   | {cv*100:.1f}% | 5-fold mean on train set |
| AUC-ROC   | {f"{auc*100:.1f}%" if auc else "N/A"} | Area under ROC curve |

**SHAP vs Feature Importance:**
- Feature Importance = how often a feature is used in splits (RF-global)
- SHAP = exact contribution of each feature to each prediction (model-local, more accurate)
- When they disagree → SHAP is more trustworthy
                """)
            ...
            st.markdown(f'<div class="bp">KMeans found <b>{best_k} natural student groups</b> '
                        f'(chosen by max silhouette score = {max(silhs):.3f}). '
                        f'Unlike manual thresholds, clustering discovers hidden patterns.</div>',
                        unsafe_allow_html=True)
            ...
            for cid, label in lmap.items():
                ...
                best_crs = df[df['Cluster'] == cid][course_cols].mean().idxmax()
                wors_crs = df[df['Cluster'] == cid][course_cols].mean().idxmin()
                st.markdown(
                    ...
                    f'<span style="color:#16a34a">Best: {best_crs}</span> &nbsp;·&nbsp; '
                    f'<span style="color:#dc2626">Weakest: {wors_crs}</span></div>',
                    unsafe_allow_html=True)
            ...
            for ax in [ax_e, ax_s]:
                ax.set_facecolor('#0f172a')
                ax.tick_params(colors='#94a3b8'); ax.spines[:].set_color('#1e3a5f')
            ...
            ax_s.bar(list(ks), silhs, color=['#d97706' if k==best_k else '#334155' for k in ks], width=.5)
            ...
            st.markdown('<div class="bb">Generates a full institutional report based on your actual data: '
                        'course averages, fail rates, RF importance, SHAP, and cluster analysis. '
                        'Different data → different report every time.</div>', unsafe_allow_html=True)
...
WEAK COURSES (RF importance + SHAP + fail rate):
{weak_str}

MEDIUM COURSES:
{mid_str}

TOP 5 MOST INFLUENTIAL COURSES (RF Feature Importance):
{top5_imp}

KMEANS CLUSTERING (best k={best_k}, silhouette={max(silhs):.3f}):
{cluster_str}
...
## 1. Executive Summary
3 sentences: key performance numbers + what clustering revealed.

## 2. Per-Course Analysis
For EACH weak course write a subsection:
### [Exact Course Name] | Fail: X% | RF: X.XXX | SHAP: X.XXXX
- **Root cause:** specific reasons for THIS course (not generic)
- **3 Teaching improvements:** concrete, course-specific methods
- **Resources:**
  * YouTube: 2–3 real, named channels/playlists for THIS topic
  * Practice: specific platform + specific track (e.g. "LeetCode → Dynamic Programming")
  * Textbook: 1–2 specific well-known books for THIS exact subject
- **Priority:** Critical / High / Medium

## 3. Cluster-Based Interventions
For each student cluster:
- Who they are, what they struggle with
- Specific intervention strategy for THIS group

## 4. SHAP vs Feature Importance Insights
Any differences between RF importance and SHAP? What does that reveal?

## 5. Priority Action Plan
Numbered, most urgent first. Include measurable targets
(e.g. "reduce fail rate in X from 40% to <25%")."""
...
with tab2:
    st.markdown('<p class="sec">02 · Job Market — TF-IDF NLP + BERT Semantic + AI Report</p>',
                unsafe_allow_html=True)
    st.markdown('<div class="bb">Upload any job listings CSV. TF-IDF extracts skills lexically. '
                'BERT (optional) adds semantic understanding. '
                'Generate Report button creates a full curriculum gap analysis.</div>',
                unsafe_allow_html=True)
    ...
    if up_j:
        ...
        curr_in  = st.text_area("Your current curriculum courses (comma-separated, for gap analysis):",
            ...
            with st.spinner("Running TF-IDF pipeline..."):
                ...
                for cat, skills in tax.items():
                    for s in skills:
                        ...
                        if freq > 0:
                            skill_r[s] = {'category':cat,'freq':freq,
...
| Stage | Action | Detail |
|-------|--------|--------|
| Stage 2: Cleanup | URL removal · punctuation · whitespace | {len(raw):,} texts |
...
BERT understands semantics — "ML engineer" matches "machine learning" even without exact words.
Combined → most complete skill demand picture.
                """)
            ...
            if has_bert:
                ...
                bars2 = ax2.barh(names, bert_s,
                                 color=['#7c3aed' if v > np.mean(bert_s) else '#334155' for v in bert_s],
                                 height=.6)
                ...
                ax_t.legend(handles=handles, fontsize=7, facecolor='#0f172a', labelcolor='#94a3b8', loc='lower right')
            ...
            if curr_in.strip():
                st.markdown("---")
                st.markdown("#### Gap Analysis — Your Curriculum vs Market")
                ...
                n_cover = len(gap_rows) - n_gaps
                g1, g2 = st.columns(2)
                g1.metric("Skills with Gap",  n_gaps,  delta_color="inverse")
                g2.metric("Skills Covered",   n_cover)
            ...
            st.markdown('<div class="bb">Generates a curriculum gap analysis + new course recommendations '
                        'based on your actual TF-IDF and BERT results. '
                        'Independent — no student data needed.</div>', unsafe_allow_html=True)
...
## 1. Market Overview
What dominates? Any TF-IDF vs BERT ranking divergence worth noting?

## 2. Per-Skill Analysis (Top 10)
For each of the top 10 skills:
### [Skill] — TF-IDF: X | Coverage: X%
- Why in demand, industry context
- How to teach it: 2 YouTube channels + 1 practice platform + 1 textbook

## 3. Curriculum–Market Gap Analysis
Based on the current curriculum provided:
- Skills with HIGH demand NOT covered (list with scores)
...
Each:
- **Course name**
- **Skills covered** (TF-IDF scores from data)
- **Market evidence** (coverage %, trend)
...
Numbered, most urgent first. Include measurable targets."""
                        ...
                        report_j = (
                            ...
                            f"Jobs={len(jobs_df)} | Skills found={len(skill_r)} | BERT={bert_status}\n\n"
                            f"TOP SKILLS:\n{top20_str}\n\nCATEGORIES:\n{cat_str}\n\n"
                            f"{'='*60}\nAI REPORT\n{'='*60}\n\n{result_j}"
                        )
                        st.download_button(
